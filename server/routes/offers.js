  ...
  try{
    ...
    if(newQty === 0){
      ...
      await createNotification(req.user.id, {
        type: 'purchase',
        message: `You bought "${offer.title}". Seller's phone: ${seller.rows[0]?.phone || 'Not reported'}`,
        offer_id: offerId
      });
...
router.post('/:id/bids/:bidId/accept', authenticate, async (req, res) => {
  try{
    ...
    // notify buyer with seller phone
    const sellerPhone = (await query('SELECT phone FROM users WHERE id=$1',[offer.seller_id])).rows[0]?.phone || 'No reported';
